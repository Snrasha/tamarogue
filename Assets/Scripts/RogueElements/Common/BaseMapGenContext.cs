...
namespace RogueElements.Examples
{
        ...
        where TMap : BaseMap, new()
    {
        ...
        public ITile RoomTerrain => new RTile(BaseMap.ROOM_TERRAIN_ID);

        public ITile WallTerrain => new RTile(BaseMap.WALL_TERRAIN_ID);
        ...
        public bool TrySetTile(Loc loc, ITile tile)
        {
            if (!this.CanSetTile(loc, tile))
                return false;
            this.Map.Tiles[loc.X][loc.Y] = (RTile)tile;
            return true;
        }
